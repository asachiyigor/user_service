...
public class RecommendationService {
    ...
    public RecommendationDto createRecommendation(RecommendationDto recommendationDto) {
        ...
        Optional<List<SkillOfferDto>> skillOffers = Optional.ofNullable(recommendationDto.getSkillOffers());
    ...
    private Recommendation getRecommendationIfExists(Long recommendationId) {
        return recommendationRepository
                ...
                .orElseThrow(() -> new DataValidationException("Recommendation Id not found. Id " + recommendationId));
    }

    private void saveSkillOffersAndGuarantee(Long recommendationId, List<SkillOfferDto> skillOffers, Long receiverId, Long
            authorId) {
        for (SkillOfferDto skillOffer : skillOffers) {
            ...
            if (skillRepository.findUserSkill(skillId, receiverId).isPresent() && !userSkillGuaranteeRepository.existsGuarantorForUserAndSkill(receiverId, skillId, authorId)) {
    ...
    private void validateRecommendationProvidedInLast6Months(Long authorId, Long receiverId, LocalDateTime
            currentDateTime) {
        Optional<Recommendation> recentRecommendation = recommendationRepository.findFirstByAuthorIdAndReceiverIdOrderByCreatedAtDesc(authorId, receiverId);
        if (recentRecommendation.isPresent() && ChronoUnit.MONTHS.between(recentRecommendation.get().getCreatedAt(), currentDateTime) <= 6) {
