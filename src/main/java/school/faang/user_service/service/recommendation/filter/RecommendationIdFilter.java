...
@Component
public class RecommendationIdFilter implements Filter<RecommendationRequest> {
    @Override
    ...
    public Stream<RecommendationRequest> apply(@NotNull Stream<RecommendationRequest> requests, RequestFilterDto filterDto) {
        return requests.filter(request -> request.getRecommendation().getId() == filterDto.getRecommendationId());
    }
}
