...
@Component
public class MessageFilter implements Filter<RecommendationRequest> {
    @Override
    public boolean isApplicable(@NotNull RequestFilterDto filterDto) {
        return filterDto.getMessage() != null && !filterDto.getMessage().isEmpty();
    }

    @Override
    public Stream<RecommendationRequest> apply(@NotNull Stream<RecommendationRequest> requests, RequestFilterDto filterDto) {
        return requests.filter(request -> request.getMessage().matches(filterDto.getMessage()));
    }
}
