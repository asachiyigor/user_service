...
public class EventParticipationServiceTest {
    ...
    public void registerParticipantThrowsExceptionIfAlreadyRegistered() {
        long eventId = 1L;
        User user = createTestUser();
        Mockito.when(eventParticipationValidator.
                        isUserRegistered(Mockito.anyLong(), Mockito.anyLong(),
                                Mockito.eq(eventParticipationRepository)))
                .thenReturn(true);
    ...
    public void unregisterParticipantThrowsExceptionIfNotRegistered() {
        ...
        when(eventParticipationValidator.isUserRegistered(eventId, exitingUser.getId(), eventParticipationRepository))
                .thenReturn(false);
    ...
    public void unregisterParticipantIsDone() {
        ...
        Mockito.when(eventParticipationValidator.isUserRegistered(Mockito.anyLong(),
                        Mockito.eq(exitingUser.getId()), Mockito.any(EventParticipationRepository.class)))
                .thenReturn(true);
    ...
    public void getParticipantsListReturnsListOfUserDto() {
        ...
        Mockito.when(userMapper.toDto(exitingUser)).thenReturn(exitingDto);

        List<UserDto> result = eventParticipationService.getParticipantsList(eventId);

        Mockito.verify(eventParticipationRepository, Mockito.times(1))
                .findAllParticipantsByEventId(eventId);
        Mockito.verify(userMapper, Mockito.times(1)).toDto(exitingUser);

        Assertions.assertEquals(resultInt, result.size());
        Assertions.assertEquals(exitingDto, result.get(0));
    }

    @Test
    public void getParticipantsCountReturnsCorrectCount() {
        int expectedCount = 5;
        long eventId = 1;
